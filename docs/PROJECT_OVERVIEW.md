# PROJECT_OVERVIEW.md — LLM Chess Arena
...
| `throttle.min_seconds_between_requests` | 4 | ≈15/min, under the 20/min cap |
...
- **Phase 3 (OpenRouter)** — live-verified 2026-07-16. That run was richer than a
  ...
  once. Per-model tuning signal captured: `gpt-oss-20b:free` proposed 27 illegal
  moves and forfeited 13 of 15 turns; `llama-3.3-70b:free` made 0 illegal moves but
  forfeited all 15 — it was rate-limited to death and never actually got to play.
- **Phase 4 (analyst)** — mock-verified; the *fallback* is live-verified. A
  model-authored verdict has still never been obtained: the account's per-minute
  limit 429s the analyst call that follows a ~100-request game.
...
- Land a model-authored commentator line and verdict — needs a looser rate-limit
  tier or a very small game (`max_moves: 6–10`) with a refilled per-minute bucket.
  This is a quota constraint, not a code gap.
